...
def json_course_checker(json_payload):
    lang = json_payload["language"]
    course_title = json_payload["title"]
    course_desc = json_payload["description"]
    course_goals = json_payload["learning_goals"]
    if lang != "en":
        return "{language no supported}"

    if course_title == "" and course_desc == "" and course_goals == "":
        return "{Course request need a Title, Description and Learning Goals}"
